...
export const CreateEditPost = () => {
  ...
  // Helper to determine if scheduleAt is a past date/time
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        ...
        // Ensure platform keys match the UI mapping keys (lowercase strings)
        setPost({
          content: data.content || "",
          platform: Array.isArray(data.platform)
            ? data.platform.map((p: string) => p.toLowerCase())
            : [],
  ...
  const handlePlatformChange = (platformKey: string) => {
    ...
    setPost((prev) => ({
      ...
      platform: prev.platform.includes(platformKey)
        ? prev.platform.filter((p) => p !== platformKey)
        : [...prev.platform, platformKey],
    }));
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div className="flex flex-wrap gap-4">
              {PLATFORMS.map((p) => {
                const isSelected = post.platform.includes(p.key);
                return (
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
            <label
              ...
              className="block text-sm font-medium text-gray-700 mb-2"
            >
              <FaImage className="inline mr-2 text-teal-600" /> Image URLa
              (optional)
            </label>
            <input
              id="imageUrl"
